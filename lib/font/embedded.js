...
class EmbeddedFont extends PDFFont {
  ...
  layoutRun(text, features) {
    ...
    console.log('chars:', JSON.stringify(run.glyphs.map(g => String.fromCharCode(g.codePoints[0])), null, 2));
    console.log('pos:', JSON.stringify(run.positions.map(p => p.xAdvance), null, 2));
  ...
  layout(text, features, onlyWidth) {
    ...
    if (text.length > 10) {
    const codepoints = text.split('').map(s => s.charCodeAt(0));
    var levels = resolve(codepoints, 0);  // [0, 0, 0, 1, 1, 1]

    let all = [];
    const dir = levels[0] === 0 ? 'ltr' : 'rtl';
    let current = {dir, chars: ''};
    for (var i = 0; i < text.length; i++) {
        const c = text[i];
        if (levels[i] !== levels[i-1] && current.chars.length > 0) {
            all.push(current);
            const dir = levels[i] === 0 ? 'ltr' : 'rtl';
            current = {dir, chars: c}
        }
        else {
            current.chars += c;
    ...
    for (let aa of all) {
        if (aa.dir === 'ltr') {
            aa.chars = aa.chars.split(' ').reverse().join(' ');
        }
    }
    let x = all.map(a => a.chars).join('');
    const revHebText = x.split(' ').reverse().join(' ');
    console.log(revHebText);
    ...
    // const run = this.layoutCached(text);
    // return { glyphs: run.glyphs, positions: run.positions, advanceWidth: run.advanceWidth };
  }
